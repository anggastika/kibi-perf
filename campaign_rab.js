...
let errorCampaignRab = new Counter("errors_CampaignRab");

const data = JSON.parse(open('./data/data.json'));

const getCampaignRab = () => {
    ...
    var res = http.get(data.baseUrl+"/campaigns/"+id+"/rab", params);

    if (res.status !== 200) {
        errorCampaignRab.add(1)
...
export default function() {
    check(getCampaignRab(), {
        "200 Get Campaign RAB By Campaign ID": r => r.status === 200
    });
    sleep(1);
}