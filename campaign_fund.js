...
let errorCampaignFund = new Counter("errors_CampaignFund");
...
const getCampaign = () => {
    ...
    var res = http.get(data.baseUrl+"/campaigns/"+id+"/fund", params);

    if (res.status !== 200) {
        errorCampaignFund.add(1)
...
export default function() {
    check(getCampaign(), {
        "200 Get Campaign Fund By Campaign ID": r => r.status === 200
    });
    sleep(1);
}