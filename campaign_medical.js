...
let errorCampaignMed = new Counter("errors_CampaignMed");

const data = JSON.parse(open('./data/data.json'));

const getCampaignMedical = () => {
    ...
    var res = http.get(data.baseUrl+"/medical/"+id, params);

    if (res.status !== 200) {
        errorCampaignMed.add(1)
...
export default function() {
    check(getCampaignMedical(), {
        "200 Get Campaign Medical By Campaign ID": r => r.status === 200
    });
    sleep(1);
}